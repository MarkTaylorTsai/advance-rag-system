...
from rag_system.backend.db import Base, JSONField, get_db, get_db_context
...
from rag_system.backend.utils.db.access_control import has_permission


log = logging.getLogger(__name__)

if Base is None or JSONField is None or get_db is None or get_db_context is None:
    raise RuntimeError("rag_system backend DB not configured. Call configure_db first.")
...
class KnowledgeTable:
    ...
    def get_knowledge_bases(
        ...
        with get_db_context(db) as db:
            ...
            users = (
                _get_model("Users").get_users_by_user_ids(user_ids, db=db)
                if user_ids
                else []
            )
    ...
    def check_access_by_user_id(
        ...
        if permission_provider is None:
            raise ValueError("permission_provider is required")
        user_group_ids = {
            group.id
            for group in _get_model("Groups").get_groups_by_member_id(user_id, db=db)
        }
        return permission_provider.has_access(
            user_id, permission, knowledge.access_control, user_group_ids, db=db
        )
    ...
    def get_knowledge_bases_by_user_id(
        ...
        return [
            ...
            or permission_provider.has_access(
                user_id, permission, knowledge_base.access_control, user_group_ids, db=db
            )
        ]
    ...
    def get_knowledge_by_id_and_user_id(
        ...
        if permission_provider is None:
            raise ValueError("permission_provider is required")
        if permission_provider.has_access(
            user_id, "write", knowledge.access_control, user_group_ids, db=db
        ):
            return knowledge
        return None
    ...
    def get_files_by_id(
        ...
        try:
            with get_db_context(db) as db:
                ...
                return [_model_dump(file) for file in files]
        except Exception:
            return []
