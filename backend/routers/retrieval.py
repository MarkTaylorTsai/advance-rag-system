...
from rag_system.backend.model_registry import get_model
from rag_system.backend.models.knowledge import Knowledges
from rag_system.backend.dependencies import (
    get_session,
    get_verified_user,
    get_admin_user,
...
from rag_system.backend.retrieval.vector.factory import VECTOR_DB_CLIENT

# Document loaders
from rag_system.backend.retrieval.loaders.main import Loader
from rag_system.backend.retrieval.loaders.youtube import YoutubeLoader

# Web search engines
from rag_system.backend.retrieval.web.main import SearchResult
from rag_system.backend.retrieval.web.utils import get_web_loader
from rag_system.backend.retrieval.web.ollama import search_ollama_cloud
from rag_system.backend.retrieval.web.perplexity_search import search_perplexity_search
from rag_system.backend.retrieval.web.brave import search_brave
from rag_system.backend.retrieval.web.kagi import search_kagi
from rag_system.backend.retrieval.web.mojeek import search_mojeek
from rag_system.backend.retrieval.web.bocha import search_bocha
from rag_system.backend.retrieval.web.duckduckgo import search_duckduckgo
from rag_system.backend.retrieval.web.google_pse import search_google_pse
from rag_system.backend.retrieval.web.jina_search import search_jina
from rag_system.backend.retrieval.web.searchapi import search_searchapi
from rag_system.backend.retrieval.web.serpapi import search_serpapi
from rag_system.backend.retrieval.web.searxng import search_searxng
from rag_system.backend.retrieval.web.yacy import search_yacy
from rag_system.backend.retrieval.web.serper import search_serper
from rag_system.backend.retrieval.web.serply import search_serply
from rag_system.backend.retrieval.web.serpstack import search_serpstack
from rag_system.backend.retrieval.web.tavily import search_tavily
from rag_system.backend.retrieval.web.bing import search_bing
from rag_system.backend.retrieval.web.azure import search_azure
from rag_system.backend.retrieval.web.exa import search_exa
from rag_system.backend.retrieval.web.perplexity import search_perplexity
from rag_system.backend.retrieval.web.sougou import search_sougou
from rag_system.backend.retrieval.web.firecrawl import search_firecrawl
from rag_system.backend.retrieval.web.external import search_external

from rag_system.backend.retrieval.utils import (
...
from rag_system.backend.retrieval.vector.utils import filter_metadata
from rag_system.backend.utils.text import (
    calculate_sha256_string,
    sanitize_text_for_db,
)
from rag_system.backend.constants import ERROR_MESSAGES

from rag_system.backend.settings import (
    ENV,
...
from rag_system.backend.env import (
    DEVICE_TYPE,
    DOCKER,
...
def get_rf(
    ...
    if reranking_model:
        if any(model in reranking_model for model in ["jinaai/jina-colbert-v2"]):
            try:
                from rag_system.backend.retrieval.models.colbert import ColBERT
            ...
            if engine == "external":
                try:
                    from rag_system.backend.retrieval.models.external import ExternalReranker
...
def process_file(
    ...
    """
    if user.role == "admin":
        file = _files().get_file_by_id(form_data.file_id, db=db)
    else:
        file = _files().get_file_by_id_and_user_id(form_data.file_id, user.id, db=db)
    ...
    if file:
        try:
            ...
            if form_data.content:
                ...
                file_path = file.path
                if file_path:
                    file_path = get_storage_provider(request).get_file(file_path)
                    loader = Loader(
            ...
            if request.app.state.config.BYPASS_EMBEDDING_AND_RETRIEVAL:
                _files().update_file_data_by_id(file.id, {"status": "completed"}, db=db)
                _files().update_file_hash_by_id(file.id, hash, db=db)
                return {
                    "status": True,
                ...
                try:
                    ...
                    if result:
                        _files().update_file_metadata_by_id(
                            file.id,
                            {
                                "collection_name": collection_name,
                            },
            ...
            _files().update_file_hash_by_id(file.id, None, db=db)
...
async def process_web_search(
    ...
    if user.role != "admin" and not get_permission_provider(request).has_permission(
