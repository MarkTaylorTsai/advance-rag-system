...
from rag_system.backend.models.knowledge import (
...
from rag_system.backend.routers.retrieval import (
    process_file,
    ProcessFileForm,
    process_files_batch,
...
async def get_knowledge_bases(
    ...
    filter = {}
    if not user.role == "admin" or not request.app.state.config.BYPASS_ADMIN_ACCESS_CONTROL:
        groups = get_model("Groups").get_groups_by_member_id(user.id, db=db)
        if groups:
            ...
            KnowledgeAccessResponse(
                **knowledge_base.model_dump(),
                write_access=(
                    ...
                    or (user.role == "admin" and request.app.state.config.BYPASS_ADMIN_ACCESS_CONTROL)
                    or get_permission_provider(request).has_access(
...
async def create_new_knowledge(
    ...
    if (
        ...
        and not get_permission_provider(request).has_permission(
            user.id,
...
async def get_knowledge_by_id(
    ...
    if knowledge:
        if (
            ...
            or get_permission_provider(request).has_access(user.id, "read", knowledge.access_control, db=db)
        ):
            ...
            return KnowledgeFilesResponse(
                **knowledge.model_dump(),
                write_access=(
                    ...
                    or (user.role == "admin" and request.app.state.config.BYPASS_ADMIN_ACCESS_CONTROL)
                    or get_permission_provider(request).has_access(user.id, "write", knowledge.access_control, db=db)
                ),
            )
        else:
            raise HTTPException(
...
async def update_knowledge_by_id(
    ...
    if (
        ...
        and not get_permission_provider(request).has_access(user.id, "write", knowledge.access_control, db=db)
        and user.role != "admin"
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def add_file_to_knowledge_by_id(
    ...
    file = get_model("Files").get_file_by_id(form_data.file_id, db=db)
    if not file:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.NOT_FOUND,
...
def remove_file_from_knowledge_by_id(
    ...
    if delete_file:
        ...
        get_model("Files").delete_file_by_id(form_data.file_id, db=db)
...
async def delete_knowledge_by_id(
    ...
    # Get all models
    models = get_model("Models").get_all_models(db=db)
    ...
    for model in models:
        if model.meta and hasattr(model.meta, "knowledge"):
            ...
            if len(updated_knowledge) != len(knowledge_list):
                ...
                model_form = get_model("ModelForm")(
                    id=model.id,
                    name=model.name,
                ...
                get_model("Models").update_model_by_id(model.id, model_form, db=db)
...
async def add_files_to_knowledge_batch(
    ...
    for form in form_data:
        file = get_model("Files").get_file_by_id(form.file_id, db=db)
        if not file:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
